...
WITH picked AS (
  ...
  WHERE e.status = 'pending'
    AND e.topic = ANY(sqlc.arg(topics)::text[])
    AND (e.next_retry_at IS NULL OR e.next_retry_at <= now())
    AND e.attempts < sqlc.arg(max_attempts)::int
  ORDER BY e.id
...
SET attempts      = e.attempts + 1,
    ...
    next_retry_at = now() + make_interval(secs => sqlc.arg(visibility_seconds)::int)
FROM picked
WHERE e.id = picked.id
RETURNING e.id, e.topic, e.payload;

-- name: FailExhaustedAnnouncements :execrows
...
-- name: AckAnnouncements :execrows
-- 確認送達 → 終態 done。
--
...
WHERE u.id = ANY(sqlc.arg(user_ids)::bigint[]) AND u.deleted_at IS NULL;
...
WHERE i.public_id = ANY(sqlc.arg(public_ids)::text[]);
