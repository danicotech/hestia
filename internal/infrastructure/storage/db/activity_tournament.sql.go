...
const getPlayerByGameID = `-- name: GetPlayerByGameID :one
...
type GetPlayerByGameIDParams struct {
	TournamentID int64
	GameID       string
}

type GetPlayerByGameIDRow struct {
	ID               int64
	PublicID         string
	TournamentID     int64
	FencerID         int64
...
// 沒有東西要比對,欄位一起拿掉(見檔頭)。
func (q *Queries) GetPlayerByGameID(ctx context.Context, arg GetPlayerByGameIDParams) (GetPlayerByGameIDRow, error) {
	row := q.db.QueryRow(ctx, getPlayerByGameID, arg.TournamentID, arg.GameID)
	var i GetPlayerByGameIDRow
	err := row.Scan(
		&i.ID,
		&i.PublicID,
		&i.TournamentID,
