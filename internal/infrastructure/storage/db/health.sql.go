...
const jobLastSuccess = `-- name: JobLastSuccess :many
...
func (q *Queries) JobLastSuccess(ctx context.Context, jobNames []string) ([]JobLastSuccessRow, error) {
	rows, err := q.db.Query(ctx, jobLastSuccess, jobNames)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []JobLastSuccessRow
	for rows.Next() {
		var i JobLastSuccessRow
		if err := rows.Scan(&i.JobName, &i.LastSuccessAt); err != nil {
			return nil, err
		}
...
const recentSessionReuse = `-- name: RecentSessionReuse :one
SELECT count(*)::bigint
...
func (q *Queries) RecentSessionReuse(ctx context.Context, windowHours int32) (int64, error) {
	row := q.db.QueryRow(ctx, recentSessionReuse, windowHours)
	var column_1 int64
