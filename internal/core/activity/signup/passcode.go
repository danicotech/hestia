...
// Hash 把明碼通行碼轉成可存進 passcode_hash 的字串。
//
...
// 必須一模一樣 —— 分辨得出來但不說出去,見 Service.Login。
...
//
// 這是「查無此遊戲ID」那條路徑唯一的存在理由:不跑的話,不存在的帳號會
// 立刻回應、存在的帳號要等幾十毫秒,於是登入頁就變成了一份可枚舉的選手名單。
// 選手名單本來就是公開的,但「誰報了名」在報名期間並不是 —— 而且這個洞
// 一旦開著,下一個接手的人很難想到要補。
